...
public class ImageManager {
	...
	public static ImageManager from(Context context) {
		...
		if (Looper.myLooper() != Looper.getMainLooper()) {
			throw new RuntimeException("不能在ui线程外实例化");
		}

		if (imageManager == null) {
			imageManager = new ImageManager(CLXApplication.getInstance());
		}
	...
	public void displayImage(ImageView imageView, String url, int resId) {
		...
		// 生成文件名
		String filePath = Utils.urlToFilePath(url);
		if (filePath == null) {
			return;
		}
	...
	private byte[] loadByteArrayFromNetwork(String url) {
		...
		try {
			...
			HttpResponse response = HttpUrlHelper.getHttpClient().execute(
					method);
